...
export class TopbarComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.authService.isAdmin() && !this.authService.isClientAdmin()) {
  ...
  loadExercice() {
    if (this.selectedSociete?.id !== undefined) {
      this.exerciceService
        .getExerciceEnCoursBySociete(this.selectedSociete.id)
        .subscribe({
          next: (data) => {
  ...
  changeSidebarWidth(): void {
    if (
      document.body.hasAttribute("data-sidebar-size") &&

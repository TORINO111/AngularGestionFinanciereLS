...
export class EtatsTresoreriesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.pageTitle = [{ label: 'trésoreries', path: '/', active: true }];
    this.init();
  }
  ...
  chargerTresoreries(): void {
    this.loading = true;
    const { societeId, from, to, codeJournal } = this.filtreForm.value;
  
    this.etatService.getEtatOperations('TRESORERIE', societeId, from, to, codeJournal)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (data) => {
  ...
  exporterPDF() {
    ...
    const { societeId, from, to ,codeJournal} = this.filtreForm.value;
    this.etatService.exportEtatPDF("TRESORERIE",societeId, from, to,codeJournal).subscribe({
      next: (blob: any) => {
        ...
        a.href = url;
        a.download = 'etat-tresoreries.pdf';
        a.click();
  ...
  exporterExcel() {
    ...
    const { societeId, from, to ,codeJournal} = this.filtreForm.value;
    this.etatService.exportEtatExcel("TRESORERIE",societeId, from, to,codeJournal).subscribe({
      next: (blob: any) => {
        clearInterval(this.intervalId);
        this.progression = 100;
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = 'etat-tresoreries.xlsx';
        a.click();
        window.URL.revokeObjectURL(url);

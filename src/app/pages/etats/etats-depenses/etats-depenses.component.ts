...
export class EtatsDepensesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.pageTitle = [{ label: 'dépenses', path: '/', active: true }];
  ...
  chargerDepenses(): void {
    ...
    this.etatService.getEtatOperations('DEPENSE', societeId, from, to, codeJournal)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (data) => {
          this.depenses = data;
  ...
  exporterPDF() {
    ...
    const { societeId, from, to ,codeJournal} = this.filtreForm.value;
    this.etatService.exportEtatPDF("DEPENSE",societeId, from, to,codeJournal).subscribe({
      next: (blob: any) => {
  ...
  exporterExcel() {
    ...
    const { societeId, from, to ,codeJournal} = this.filtreForm.value;
    this.etatService.exportEtatExcel("DEPENSE",societeId, from, to,codeJournal).subscribe({
      next: (blob: any) => {
        clearInterval(this.intervalId);
        this.progression = 100;

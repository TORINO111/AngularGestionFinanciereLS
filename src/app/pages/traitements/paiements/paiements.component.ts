...
import { NatureOperation } from 'src/app/models/operation.model';
...
export class PaiementsComponent implements OnInit {
  ...
  chargerToutesLesDonnees(): void {
    ...
    forkJoin({
      operations: this.operationService.getByFilters(1, 'DECAISSEMENT', 'TRESORERIE'),
      tiers: this.tiersService.getBySocieteAndType(1, 'FOURNISSEUR'),
      natureOperations: this.natureOperationService.getByFilters(1, 'DECAISSEMENT', 'TRESORERIE')
    }).subscribe({
  ...
  chargerOperations(): void {
    this.operations = [];
    this.operationService.getByFilters(1, 'DECAISSEMENT', 'TRESORERIE').subscribe({
      next: (data: Operation[]) => {
        console.log(data)
  ...
  chargerTiers() {
    this.tiersService.getBySocieteAndType(1,'FOURNISSEUR').subscribe(
      (data:any) => {
        for(let d of data){
          console.log(data)
  ...
  chargerNatureOperations() {
    this.natureOperationService.getByFilters(1,'DECAISSEMENT','TRESORERIE').subscribe(
      (data:any) => {
        for(let d of data){
          //console.log(data)

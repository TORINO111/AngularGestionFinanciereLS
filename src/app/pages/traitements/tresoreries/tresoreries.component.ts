...
export class TresoreriesComponent implements OnInit {
  ...
  chargerToutesLesDonnees(): void {
    ...
    forkJoin({
      operations: this.operationService.getByFilters(1, 'ENCAISSEMENT', 'TRESORERIE'),
      tiers: this.tiersService.getBySocieteAndType(1, 'CLIENT'),
      natureOperations: this.natureOperationService.getByFilters(1, 'ENCAISSEMENT', 'TRESORERIE')
    }).subscribe({
  ...
  chargerOperations(): void {
    this.operations = [];
    this.operationService.getByFilters(1, 'ENCAISSEMENT', 'TRESORERIE').subscribe({
      next: (data: Operation[]) => {
        console.log(data)
  ...
  chargerTiers() {
    this.tiersService.getBySocieteAndType(1,'CLIENT').subscribe(
      (data:any) => {
        for(let d of data){
          console.log(data)
  ...
  chargerNatureOperations() {
    this.natureOperationService.getByFilters(1,'ENCAISSEMENT','TRESORERIE').subscribe(
      (data:any) => {
        for(let d of data){
          //console.log(data)

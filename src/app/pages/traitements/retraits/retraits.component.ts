...
export class RetraitsComponent implements OnInit {
  ...
  chargerOperations(): void {
    this.operations = [];
    this.operationService.getByFilters(1, 'RETRAIT', 'TRESORERIE').subscribe({
      next: (data: Operation[]) => {
        console.log(data)
  ...
  //       for(let d of data){
  //         //console.log(data)
  //         this.natureOperations = [{ label: '', options: (data as OperationDto[]).map(d => ({ value: d.id, label: d.libelle })) }];
  //       }
  //       this.isLoading=true;
  //     },
  //     (error) => {

...
export class OperationsComponentBi implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.chargerComptesAchats();
    this.chargerComptesVentes();
  ...
  onTypeOperationChange(){
    const typeOperation = this.operationForm.get("typeOperation")?.value;
    ...
    // filtrer les articles compatibles avec le type d'opération sélectionné
    this.filteredArticles = this.articles.filter((a) =>
      a.comptesParCategorie?.some((c: { typeOperation: any; }) => c.typeOperation === typeOperation)
    );
  ...
  chargerComptesAchats() {
    this.compteComptableService.getAllCompteAchats().subscribe( {
      next:(data: CompteComptableDTO[]) => {
        this.comptesAchats = data;
      },
      error: (error: any) => {
        console.log("Erreur lors du chargement des comptes d'achats", error);
        this.notification.showError(
          "Erreur lors du chargement des comptes d'achats"
        );
      },
    }
    );
  ...
  chargerComptesVentes() {
    this.compteComptableService.getAllCompteVentes().subscribe( {
      next: (response: any) => {
        this.comptesVentes = response;
      },
      error: (error: any) => {
        console.log("Erreur lors du chargement des comptes de ventes", error);
        this.notification.showError(
          "Erreur lors du chargement des comptes de ventes"
        );
      },
    }
    );

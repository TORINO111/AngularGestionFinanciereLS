...
export class CompteComponent implements OnInit {
  ...
  constructor(private router: Router,private tresorerieService: TresorerieService,private authenticationService:AuthenticationService,private fb: UntypedFormBuilder,private toastr: ToastrService) {
   ...
   loadAndInitUserForm(): void {
    this.tresorerieService.getUserById(this.user?.id).subscribe({
      next: (userData) => {
        this.user = userData;
        this.initForm();
        ...
        this.userForm.get('telephone')?.valueChanges.subscribe((value) => {
          if (this.userForm.get('telephone')?.valid && value) {
            ...
            this.userForm.get('telephone')?.setValue(formatted, { emitEvent: false });
          }
        });
        this.result=true;
      },
...
validatePhoneNumber(control: UntypedFormControl): { [key: string]: any } | null {
  ...
  const isValid = cleaned.length === 8 || // 70123456
...
formatPhoneNumber(phoneNumber: string): string {
  ...
  if (cleaned.length === 8) {
    return `+226 ${cleaned.substring(0, 2)} ${cleaned.substring(2, 4)} ${cleaned.substring(4, 6)} ${cleaned.substring(6)}`;
  }
  else if (cleaned.startsWith('226') && cleaned.length === 11) {
    return `+226 ${cleaned.substring(3, 5)} ${cleaned.substring(5, 7)} ${cleaned.substring(7, 9)} ${cleaned.substring(9)}`;
  }
  else if (cleaned.startsWith('00226') && cleaned.length === 13) {
    return `+226 ${cleaned.substring(5, 7)} ${cleaned.substring(7, 9)} ${cleaned.substring(9, 11)} ${cleaned.substring(11)}`;
  }
  return phoneNumber;
}
  ngOnInit(): void {
  ...
  loadCurrentUser(id:number){
    this.tresorerieService.getUserById(id).subscribe(
      {
  ...
  sendPassword(passwordForm:any){
    ...
    if(!(passwordForm.value.password===passwordForm.value.confirmation)){
      this.showWarning('Le mot de passe et la confirmation ne sont pas identiques')
      this.loading=false;
      return;
    }

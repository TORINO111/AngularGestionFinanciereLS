...
export class AuthenticationService {
  ...
  refreshTokenRequest(): Observable<any> {
    return this._http.post(`${this.host}/auth/refresh`, {
  ...
  isClientAdmin() {
    return sessionStorage.getItem("role") === "CLIENT_ADMIN";
  }
  isClientComptable() {
    return sessionStorage.getItem("role") === "CLIENT_COMPTABLE";
  }
  isClientAgent() {
    return sessionStorage.getItem("role") === "CLIENT_AGENT";
  }
  isBailleur() {
    return sessionStorage.getItem("role") === "BAILLEUR";
  }
  isEntrepriseUser() {
    return sessionStorage.getItem("role") === "ENTREPRISE_USER";
  }
  isEntrepriseAdmin() {
    return sessionStorage.getItem("role") === "ENTREPRISE_ADMIN";
  }
  ...
  loginn(email: string, password: string): any {
    return this._http
      .post<any>(`${this.host}/auth/login`, { email, password })
      .pipe(
        map((user) => {
          if (user && user.token) {
            this.user = user;
  ...
  signup(name: string, email: string, password: string): any {
    return this._http
      .post<any>(`${this.host}/signup`, { name, email, password })
      .pipe(map((user) => user));
  }
  ...
  getUserByUsername(username: string) {
    ...
    return this._http.get(
      `${this.host}/utilisateurs/utilisateur-username?username=${username}`,
      {
  ...
  sendNewPassword(password: any, token: any) {
    return this._http.post(
      `${this.host}/reset?password=${password}&token=${token}`,
      {},
      {
        headers: new HttpHeaders().set("Content-Type", "application/json"),
      }
  ...
  sendPassword(payload: any) {
    return this._http.post(`${this.host}/auth/new-password`, payload, {
